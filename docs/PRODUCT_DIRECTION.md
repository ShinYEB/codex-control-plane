...
이 문서는 RUVORA가 해결하려는 사용자 문제와 제품 경계를 정의하는 정본이다. 실행 방법은 [ARCHITECTURE.md](./ARCHITECTURE.md), 세부 계약은 [설계 문서 인덱스](./README.md)를 따른다.
...
> RUVORA는 여러 프로젝트와 스레드의 지식·상태를 색인하고, 목표에 맞는 맥락을 근거와 함께 구성하며, 이를 바탕으로 안전한 다중 스레드·다중 프로젝트 실행을 지휘하는 로컬 영속 조정 계층이다.
